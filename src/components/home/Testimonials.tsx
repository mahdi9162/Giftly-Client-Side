  ...
  return (
    <Container>
      <section className="py-16 md:py-20 px-3 md:px-0">
        ...
        {/* Framed wrapper */}
        <div className="relative overflow-hidden rounded-4xl border border-slate-200/70 bg-linear-to-br from-white via-rose-50/20 to-violet-50/20 shadow-[0_24px_80px_rgba(15,23,42,0.06)] p-4 sm:p-6 lg:p-10">

          {/* Corner glow blobs */}
          <div className="pointer-events-none absolute -left-16 top-10 h-40 w-40 rounded-full bg-rose-200/20 blur-3xl" />
          <div className="pointer-events-none absolute -right-16 bottom-10 h-40 w-40 rounded-full bg-violet-200/15 blur-3xl" />
            ...
            >
              {testimonials.map((item) => (
                <SwiperSlide key={item.id} className="h-auto">
                  <div className="group flex h-full flex-col rounded-2xl border border-slate-200/60 bg-white p-6 shadow-[0_4px_16px_-4px_rgba(15,23,42,0.07)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_32px_-8px_rgba(15,23,42,0.11)]">

                    {/* Quote icon */}
                    <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/8 text-primary/60">
                      <Quote className="h-6 w-6" strokeWidth={2} />
                    </div>

                    {/* Review text */}
                    <p className="flex-1 text-sm md:text-base md:leading-7 text-slate-600">
